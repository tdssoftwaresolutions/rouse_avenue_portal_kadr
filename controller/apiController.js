...
module.exports = {
  ...
  markCaseResolved: async function (req, res) {
    try {
      const { caseId, bothAgreed, agreementText, signature } = req.body
      if (!caseId || typeof bothAgreed === 'undefined' || !agreementText || !signature) {
      ...
      const agreementRecord = await prisma.case_agreement_tracking.create({
        data: {
          both_parties_agreed: bothAgreed,
      ...
      await prisma.cases.update({
        where: { id: caseId },
        data: { case_agreement: agreementRecord.id, status: CaseTypes.CLOSED_SUCCESS, sub_status: null }
      })
      ...
      const htmlBody = `
     <p>Hi ${caseRecord.user_cases_first_partyTouser.name}, <br/> Congratulations, the medidation Rouse Avenue Court is now resolved.<br/>You are the first party in this mediation case. To proceed with the final agreement, we require your signature verification. <br/> Please click the link below to review and provide your signature:</p>
  ...
  submitAgreementSignature: async function (req, res) {
    try {
      ...
      if (generateAgeement === true) {
        ...
        // Pull events for session dates
        const events = await prisma.events.findMany({
          where: { case_id: caseRecord.id },
          select: { id: true, start_datetime: true }
        })
        const sessionDates = events.map(e => e.start_datetime)
        const numberOfSessions = events.length
        ...
        // --- Begin: Generate PDF in temp file, upload to S3, then cleanup ---
        const os = require('os')
