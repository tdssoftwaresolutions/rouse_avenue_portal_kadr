  ...
  jQuery(document).ready(function () {
    jQuery(window).on('load', function (e) {
      ...
      function headerHeight () {
        let height = jQuery('#main-header').height()
    ...
    Screenshots silder
    -------------------------- */
      let slide = jQuery('.slider-single')
      ...
      function slideRight () {
        ...
        let activeSlide = slide.eq(slideCurrent)
        if (slideCurrent < slideTotal) {
      ...
      let left = jQuery('.slider-left')
      let right = jQuery('.slider-right')
      ...
      /* ------------------------
            Masonry
            -------------------------- */
      let jQuerymsnry = jQuery('.iq-masonry-block .iq-masonry')
      if (jQuerymsnry) {
        let jQueryfilter = jQuery('.iq-masonry-block .isotope-filters')
        jQuerymsnry.isotope({
        ...
        jQueryfilter.each(function (i, buttonGroup) {
          let jQuerybuttonGroup = jQuery(buttonGroup)
    ...
    jQuery(window).on('scroll', function (e) {
      let nav = jQuery('#pills-tab')
      if (nav.length) {
        let contentNav = nav.offset().top - window.outerHeight
    ...
    /* ---------------------------
        ...
        --------------------------- */
    jQuery(document).ready(function () {
      let a = jQuery('.nav.nav-pills').each(function () {
        let b = jQuery(this).find('a.active').attr('href')
        activaTab(b)
      })
    })
    ...
    function animateElements () {
      jQuery('.progressbar-circle').each(function () {
        let elementPos = jQuery(this).offset().top
        let topOfWindow = jQuery(window).scrollTop()
        let percent = jQuery(this).find('.circle').attr('data-percent')
        let percentage = parseInt(percent, 10) / parseInt(100, 10)
        let animate = jQuery(this).data('animate')
    ...
    /* ---------------------------
        Pricing
        --------------------------- */
    let count_tabs = jQuery('#count-tabs').val()
    ...
    jQuery('.input-range').on('click mousemove', function () {
      let check = jQuery(this).attr('id')

      let id = jQuery(this).attr('id').match(/\d+/)

      let user_range = jQuery('#user_range_' + id)
      let time_range = jQuery('#time_range_' + id)

      let user_text = jQuery('#user_text_' + id)

      let time_text = jQuery('#time_text_' + id)

      let total = jQuery('#total_' + id)
      let base = jQuery('#base_' + id)
    ...
    jQuery('.price-checkbox').on('click', function () {
      let a = []
      let id = jQuery(this).attr('primary').match(/\d+/)
      let price = 0
      let user_range = jQuery('#user_range_' + id)
      let time_range = jQuery('#time_range_' + id)
